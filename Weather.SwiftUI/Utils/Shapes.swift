...
struct Arc: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let movePoint = CGPoint(x: rect.minX - 1, y: rect.minY)
        path.move(to: movePoint)
        
        let maxXPoint = CGPoint(x: rect.maxX + 1, y: rect.minY)
        let control = CGPoint(x: rect.midX, y: rect.midY)
        path.addQuadCurve(to: maxXPoint, control: control)
        
        let linePointOne = CGPoint(x: rect.maxX + 1, y: rect.maxY + 1)
        let linePointTwo = CGPoint(x: rect.minX - 1, y: rect.maxY + 1)
        path.addLine(to: linePointOne)
        path.addLine(to: linePointTwo)
        path.closeSubpath()
